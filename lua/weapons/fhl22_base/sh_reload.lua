...
function SWEP:Reload()
	...
	if SERVER and self.Stats["Magazine"]["Ammo reloaded"] <= 0 then self:GetOwner():DropWeapon(self) return end
	if math.min( self:Ammo1(), self.Stats["Magazine"]["Ammo loaded max"] - self:Clip1() ) <= 0 then return end
...
function SWEP:Load()
	local thing = math.min( self:Ammo1(), self.Stats["Magazine"]["Ammo reloaded"], self.Stats["Magazine"]["Ammo loaded max"] - self:Clip1() )
